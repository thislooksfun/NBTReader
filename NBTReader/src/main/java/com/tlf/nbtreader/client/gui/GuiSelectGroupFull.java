...
public class GuiSelectGroupFull extends Gui
{
	...
	public GuiSelectGroupFull(int top, int left, int width, int height, ArrayList<String> data)
	{
		...
		this.done = new GuiOtherButton(30, 11, "Done").setColor(Colors.rgb(0, 200, 0));
		this.add = new GuiOtherButton(30, 11, "+").setColor(Colors.rgb(0, 200, 0));
	...
	{
		GuiOtherButton b = new GuiOtherButton(8, 8, "-").setColor(Colors.rgb(200, 0, 0));
		b.left = this.left + 6;
	...
	{
		this.rect(5, 5, this.width - 10, this.height - 10, Colors.rgb(150, 100, 100));
		this.done.render();
		this.add.render();
	}
	...
	{
		this.rect(this.width - 9, 24, 4, this.height - 29, Colors.rgb(0, 150, 0));
		
		int height = 15;
		float percent = ((float)this.start / (this.fields.size() + 1 - PERPAGE));
		int offset = (int)(percent * (this.height - 31 - height));
		
		this.rect(this.width - 8, 25 + offset, 2, height, Colors.rgb(0, 255, 124));
	}
