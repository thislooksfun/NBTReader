 ...
 * @author thislooksfun
 */
public class GuiNBTBase extends GuiContainer
{
	private static final ResourceLocation img = new ResourceLocation(NBTReader.MODID, "textures/gui/reader.png");
	
	private ArrayList<String> displayStrings = new ArrayList<String>();
	private int start = 0;
	
	public GuiNBTBase(ContainerNBTBase container)
	{
		super(container);
		this.xSize = 224;
		this.ySize = 233;
	...
	protected void drawGuiContainerForegroundLayer(int p_146979_1_, int p_146979_2_)
	{
		...
		drawRect(5, 5, this.xSize-5, 146, Colors.rgba(0, 11, 121, 255));
		drawRect(5, 5, this.xSize-5, 17, Colors.rgba(207, 0, 0, 255));
	...
	{
		drawRect(this.xSize - 9, 17, this.xSize - 5, 146, Colors.rgba(0, 150, 0, 255));
		
		int height = 15;
		float percent = ((float)this.start / (this.displayStrings.size() - 14));
		int offset = (int)(percent*(127-height));
		
		drawRect(this.xSize - 8, 18 + offset, this.xSize - 6, 18 + offset + height, Colors.rgba(0, 255, 124, 255));
	}
...
//		fullStr = fullStr.substring(1, fullStr.length()-1).replace("\n", "\\n");
		...
		for (int i = 0; i < fullStr.length(); i++)
		{
			try
			{
				...
				{
//					LogHelper.info("\t* Checking char: " + c);
					if (c == '{' || c == '[')
					{
						if (s.length() > 0)
						{
